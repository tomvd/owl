...
/**
 * CRC-16 implementation for Davis Vantage Pro protocol validation.
 * <p>
 * Davis uses CRC-CCITT polynomial with table lookup for efficiency.
 */
public class CRC16 {
     ...
     *
     * @param data byte array to calculate CRC for
     * @return 2-byte CRC (MSB first)
     */
    ...
    /**
     * Check if data with embedded CRC is valid.
     * CRC is assumed to be at the end of the data segment.
     *
     * @param data   buffer containing data and CRC
    ...
    /**
     * Check if data matches separate CRC bytes.
     *
     * @param data data bytes
     * @param crc  expected CRC (2 bytes)
    ...
    /**
     * Add multiple bytes to the CRC calculation.
     */
    public void add(byte[] b) {
        for (byte value : b) {
            add(value);
    ...
    /**
     * Check if the accumulated CRC is valid (zero).
     */
    ...
    /**
     * Get the current CRC value as 2 bytes (MSB first).
     */
    public byte[] getCrc() {
